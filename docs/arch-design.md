...
사용자가 PPT 템플릿과 문서를 올리면, 프론트가 Job을 생성하고, 백엔드가 비동기 워커를 실행해 템플릿 스타일을 유지한 결과 PPT를 생성하는 구조다.
...
8. Worker가 문서 파싱 및 산출물 생성
9. 프론트가 GET /jobs/{jobId} 폴링
10. 완료 시 결과 PPT 다운로드
```
...
- 워커 후반 단계가 아직 완성되지 않았다.
- 실제 업로드 흐름은 최종 PPT 생성까지 아직 닫히지 않았다.
- 데모 fast-path는 피드백 수집용이며 장기적인 최종 구조는 아니다.
