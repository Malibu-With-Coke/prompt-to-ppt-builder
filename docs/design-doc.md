...
8. 워커가 템플릿 PPT와 콘텐츠 문서를 파싱한다.
9. LLM이 템플릿의 각 텍스트 shape에 들어갈 새 문구를 생성한다.
10. 워커가 기존 템플릿 슬라이드의 shape를 직접 치환해 결과 PPT를 만든다.
11. 프론트가 `GET /jobs/{jobId}`를 폴링하며 상태를 표시하고 완료 시 다운로드한다.
...
- Excel 차트 자체를 템플릿의 native chart 데이터로 갱신하는 기능은 아직 제한적이다.
